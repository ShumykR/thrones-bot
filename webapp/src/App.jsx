    ...
    const fetchData = async () => {
      try {
        ...
        if (initData) {
          ...
          // Mock data
          setTimeout(() => {
            setMe({ user_id: 1, first_name: 'Jon Snow', role: 'lord', army_size: 1500, castles_count: 1, independence_points: 0 });
            setGameState({
        ...
        if (tg) tg.showAlert("Помилка з'єднання з Воронячою поштою.");
      }
      setLoading(false);
    };
  ...
  const handleAttack = async (castleId, amount) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setMe(prev => ({ ...prev, army_size: prev.army_size - amount }));
      } else {
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'map' && (
            <motion.div
              ...
              style={{ height: '100%' }}
            >
              <MapView state={gameState} me={me} onAttack={handleAttack} tg={tg} />
            </motion.div>
          )}
          ...
          {activeTab === 'throne' && (
            <motion.div
              ...
              style={{ height: '100%' }}
            >
              <ThroneView state={gameState} me={me} tg={tg} />
            </motion.div>
          )}
          ...
          {activeTab === 'profile' && (
            <motion.div
              ...
              style={{ height: '100%' }}
            >
              <ProfileView me={me} state={gameState} tg={tg} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
